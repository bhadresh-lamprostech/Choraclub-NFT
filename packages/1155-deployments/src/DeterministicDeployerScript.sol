...
contract DeterministicDeployerScript is Script {
    ...
    function mineSalt(address deployer, bytes32 initCodeHash, string memory startsWith) internal returns (bytes32 salt, address expectedAddress) {
        ...
        uint256 saltEndIndex = LibString.indexOf(result, " (", saltIndex);
        string memory saltStr = LibString.slice(result, saltIndex + 6, saltEndIndex);
    ...
    function getProxyParams(
        ...
        (proxySalt, deterministicProxyAddress) = mineSalt(proxyDeployerAddress, creationCodeHash, "111111");
    }
